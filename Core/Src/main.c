...
osThreadId_t LedsControlHandle;
const osThreadAttr_t LedsControl_attributes = {
    .name = "LedsControl",
    .stack_size = 128 * 4,
    .priority = (osPriority_t) osPriorityNormal,
};
...
/* Definitions for personnalDelays */
osThreadId_t gereMicHandle;
const osThreadAttr_t gereMic_attributes = {
    .name = "gereMic",
    .stack_size = 128 * 4,
    .priority = (osPriority_t) osPriorityNormal,
};
...
/* Definitions for potenValue */
osMessageQueueId_t micValueHandle;
const osMessageQueueAttr_t micValue_attributes = {
    .name = "micValue"
};

/* Definitions for semaphoreTest */
osSemaphoreId_t semaphoreValidMicroHandle;
const osSemaphoreAttr_t semaphoreValidMicro_attributes = {
    .name = "semaphoreValidMicro"
};

/* Definitions for stateDiagram */
osThreadId_t gereScoresHandle;
const osThreadAttr_t gereScores_attributes = {
    .name = "gereScores",
    .stack_size = 128 * 4,
    .priority = (osPriority_t) osPriorityNormal,
};
...
/* Definitions for semaphoreTest */
osSemaphoreId_t semaphoreDisplayScoresHandle;
const osSemaphoreAttr_t semaphoreDisplayScores_attributes = {
    .name = "semaphoreDisplayScores"
};

/* Definitions for semaphoreTest */
osSemaphoreId_t semaphoreEtatHautHandle;
const osSemaphoreAttr_t semaphoreEtatHaut_attributes = {
    .name = "semaphoreEtatHaut"
};
...
int ignoreSemaphore = 0b11;
uint16_t potenValue = 0;
uint16_t micValue = 0;
char potenValue_string[20];
char micValue_string[20];
...
int validMic = 0;
int bestRecord_temp = 0;
int timer_temp = 0;
int cpt_valMax = 0;
int oldValueMic = 0;

int bestRecordJeu1 = 0;
int bestRecordJeu2 = 0;
char bestRecordJeu1_string[20];
char bestRecordJeu2_string[20];

int etatLeds = 0;

size_t screenSize = sizeof("HUMAN SPEED TEST") - 1;
...
void LedsControl(void * argument);
...
void menuLCD(void);
void getValueMic(void * argument);
void gereScores(void * argument);
...
void displayTest1(void) {
    ignoreSemaphore = 0b11;
    LCD_clear();

    LCD_home();
    LCD_setCursor(0, 0);
    LCD_print("      JEU 1     ", screenSize);
    HAL_GPIO_WritePin(GPIOB, GPIO_PIN_10, 1);
    HAL_GPIO_WritePin(GPIOA, GPIO_PIN_8, 1);
    HAL_GPIO_WritePin(GPIOA, GPIO_PIN_9, 1);
    LCD_setCursor(0, 1);
    LCD_print("                ", screenSize);
    osDelay(1000);
    HAL_GPIO_WritePin(GPIOA, GPIO_PIN_8, 0);
    HAL_GPIO_WritePin(GPIOA, GPIO_PIN_9, 0);
    LCD_setCursor(0, 0);
    LCD_print("        3       ", screenSize);
    osDelay(1000);
    LCD_setCursor(0, 0);
    LCD_print("        2       ", screenSize);
    HAL_GPIO_WritePin(GPIOA, GPIO_PIN_9, 1);
    osDelay(1000);
    LCD_setCursor(0, 0);
    LCD_print("        1       ", screenSize);
    HAL_GPIO_WritePin(GPIOA, GPIO_PIN_8, 1);
    osDelay(1000);
    ...
    HAL_GPIO_WritePin(GPIOA, GPIO_PIN_8, 1);
    LCD_setCursor(0, 0);
    LCD_print("     PUSH       ", screenSize);
    bestRecord = 0;
    TIM2 -> CNT = 0;
    cpt = 0;
    ignoreSemaphore = 0b01;
    int iVerif = osSemaphoreAcquire(semaphoreEtatHautHandle, 5000);
    ignoreSemaphore = 0b11;
    if (iVerif == osOK) {
        ...
        LCD_setCursor(0, 1);
        LCD_print(aye_string, screenSize);
        osDelay(5000);
        if (cpt < bestRecordJeu1) {
            bestRecordJeu1 = cpt;
            strcpy(bestRecordJeu1_string, aye_string);
        }

    } else if (iVerif == osErrorTimeout) {
        LCD_setCursor(0, 1);
        LCD_print("   TROP LENT   ", screenSize);
        osDelay(5000);
    }
    LCD_setCursor(0, 0);
    LCD_print("  PRESS - PLAY  ", screenSize);
    LCD_setCursor(0, 1);
    LCD_print("   WAIT - MENU  ", screenSize);
    ignoreSemaphore = 0b10;
    iVerif = osSemaphoreAcquire(semaphoreTestHandle, 5000);
    ignoreSemaphore = 0b11;
    if (iVerif == osErrorTimeout) {
        etatCourant = MENU;
    }
}
...
void displayTest2(void) {
    ...
    osSemaphoreAcquire(semaphoreValidMicroHandle, 1);
    LCD_setCursor(0, 0);
    LCD_print("     PUSH       ", screenSize);
    bestRecord = 0;
    TIM2 -> CNT = 0;
    ...
    validMic = 0;
    cpt_valMax = 0;
    osSemaphoreAcquire(semaphoreValidMicroHandle, 5000);
    int iVerif = 0;
    if (iVerif == osOK) {

        cpt = (bestRecord_temp * 1000000) + (timer_temp / 4);
        aye_res = cpt;

        sprintf(aye_string, "%d", aye_res);
        LCD_setCursor(0, 1);
        LCD_print(aye_string, sizeof(aye_string) - 1);
        osDelay(5000);
        if (cpt < bestRecordJeu2) {
            bestRecordJeu2 = cpt;
            strcpy(bestRecordJeu2_string, aye_string);
        }
...
void startingLCD(void) {
    ...
    LCD_home();
    LCD_setCursor(0, 0);
    LCD_print("HUMAN SPEED TEST", screenSize);
    //LCD_setCursor(0, 1);
    osDelay(1000);
    ...
    LCD_setCursor(0, 1);
    LCD_print("-- JOS MAR DAV--", screenSize);
    osDelay(1000);
    ...
    LCD_setCursor(0, 0);
    LCD_print("     Press      ", screenSize);
    LCD_setCursor(0, 1);
    LCD_print("  to continue   ", screenSize);
}
...
void menuLCD(void) {
    ...
    osMessageQueueGet(potenValueHandle, & potenValue, 0, 10);
    ...
    if (abs((potenValue - potenValueTemp)) > 10) {
        ...
        if (potenValue < 1024) {
            ...
            LCD_print("     JEU 1      ", screenSize);
            LCD_setCursor(0, 1);
            LCD_print("   X  0  0  0   ", screenSize);
            idGame = 1;
        } else if (potenValue < 2048) {
            LCD_setCursor(0, 1);
            LCD_print("   0  X  0  0   ", screenSize);
            LCD_setCursor(0, 0);
            LCD_print("     JEU 2      ", screenSize);
            idGame = 2;
        } else if (potenValue < 3072) {
            LCD_setCursor(0, 1);
            LCD_print("   0  0  X  0   ", screenSize);
            LCD_setCursor(0, 0);
            LCD_print("     JEU 3      ", screenSize);
            idGame = 3;
        } else {
            LCD_setCursor(0, 1);
            LCD_print("   0  0  0  X   ", screenSize);
            LCD_setCursor(0, 0);
            LCD_print("     SCORES     ", screenSize);
            idGame = 4;
        }
        osDelay(1);
    }
...
void displayScores(void) {
    ...
    if (osThreadGetState(gereScoresHandle) == osThreadBlocked) {
        osThreadResume(gereScoresHandle);
    }
    int iVerif = osSemaphoreAcquire(semaphoreTestHandle, 1);
    if(iVerif == osErrorTimeout){
        if (abs((potenValue - potenValueTemp)) > 10) {
                potenValueTemp = potenValue;
                if (potenValue < 1024) {
                        LCD_setCursor(0, 1);
                        LCD_print(bestRecordJeu1_string, sizeof(bestRecordJeu1_string)-1);

                    idGame = 1;
                } else if (potenValue < 2048) {
                        LCD_setCursor(0, 1);
                        LCD_print(bestRecordJeu2_string, sizeof(bestRecordJeu2_string)-1);
        ...
        osThreadSuspend(gereScoresHandle);
        LCD_clear();
        etatCourant = MENU ;
    }
    osDelay(1);
...
int main(void) {
    ...
    semaphoreValidMicroHandle = osSemaphoreNew(1, 0, & semaphoreValidMicro_attributes);

    semaphoreDisplayScoresHandle = osSemaphoreNew(1, 0, & semaphoreDisplayScores_attributes);

    semaphoreEtatHautHandle = osSemaphoreNew(1, 0, & semaphoreEtatHaut_attributes);
    ...
    micValueHandle = osMessageQueueNew(16, sizeof(uint16_t), & micValue_attributes);
    /* add queues, ... */
    ...
    LedsControlHandle = osThreadNew(LedsControl, NULL, & LedsControl_attributes);
    ...
    /* creation of personnalDelays */
    gereMicHandle = osThreadNew(getValueMic, NULL, & gereMic_attributes);

    /* creation of stateDiagram */
    gereScoresHandle = osThreadNew(gereScores, NULL, & gereScores_attributes);
...
void GereLeds_1(void * argument) {
    ...
    int buttonRelease = 1;
    int buttonReleaseEtatHaut = 1;
    /* Infinite loop */
    for (;;) {

        if (HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_10) == 1) {
            buttonRelease = 1;
            if (buttonReleaseEtatHaut == 1) {
                buttonReleaseEtatHaut = 0;
                if ((ignoreSemaphore & 0b10) == 0) {
                    osSemaphoreRelease(semaphoreEtatHautHandle);
                }
            }
        } else {
            buttonReleaseEtatHaut = 1;
            if (buttonRelease == 1) {
                buttonRelease = 0;
                if ((ignoreSemaphore & 0b01) == 0) {
...
//        LCD_setCursor(0, 0);
//        LCD_print("azeazeaze", sizeof("azeazeaze")-1);
//        osDelay(1);
        osMessageQueueGet(potenValueHandle, & potenValue, 0, osWaitForever);
        switch (idGame) {
            case 1:
...
void changeRGBs(void * argument) {
    ...
    for (;;) {
        ...
        if ((etatCourant == STARTING) || (etatCourant == MENU)) {
            defilColors();
        }
        /*
              else if{
...
void getValueMic(void * argument) {
    ...
    for (;;) {
        ...
        micValue = HAL_ADC_GetValue( & hadc2);
        sprintf(micValue_string, "%d", micValue);
        /*LCD_setCursor(0, 0);
        LCD_print(micValue_string, sizeof(micValue_string)-1);*/

        if (micValue > 1500) {
            if (cpt_valMax == 0) {
                bestRecord_temp = bestRecord;
                timer_temp = TIM2 -> CNT;
            }

            cpt_valMax++;
            if (cpt_valMax == 4) {
                osSemaphoreRelease(semaphoreValidMicroHandle);
                cpt_valMax = 0;
            }
        } else {
            cpt_valMax = 0;
